...
async function create(mission, { reporter } = {}) {
  ...
  try {
    runGit(projectDir, ['remote', 'get-url', 'origin']);
  } catch {
    throw new Error(
      'No git remote "origin" configured. Add one with: git remote add origin <url>'
    );
  }

  // Ensure SSH remote (gh clone defaults to HTTPS which won't work in containers)
  worktree.ensureSshRemote(projectDir);
  ...
  try {
    ...
    throw new Error(
      `Failed to push branch "${integrationBranch}" to origin. ` +
      `Verify you have push access and the remote is reachable. ` +
      `Details: ${err.message}`
    );
  }

  // Create the PR via gh CLI — write body to temp file for container compatibility
  const bodyFile = path.join(os.tmpdir(), `commanddeck-pr-body-${Date.now()}.md`);
  let prUrl;
  try {
...
async function updateBody(mission) {
  ...
  const bodyFile = path.join(os.tmpdir(), `commanddeck-pr-edit-${Date.now()}.md`);
  try {
    fs.writeFileSync(bodyFile, prBody, 'utf-8');
