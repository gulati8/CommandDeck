...
function buildPRBody(mission) {
  ...
  // Slack thread metadata for GH Actions to post UAT URL back
  if (mission.slack_channel && mission.slack_thread_ts) {
    body += `\n<!-- commanddeck:slack_channel=${mission.slack_channel} -->\n`;
    body += `<!-- commanddeck:slack_thread_ts=${mission.slack_thread_ts} -->\n`;
  }
