...
// Ensure the origin remote uses SSH (git@github.com:...) instead of HTTPS.
// gh repo clone defaults to HTTPS, but the container authenticates via SSH keys.
function ensureSshRemote(projectDir) {
  try {
    const url = runGit(projectDir, ['remote', 'get-url', 'origin']).trim();
    const match = url.match(/^https:\/\/github\.com\/(.+?)(?:\.git)?$/);
    if (match) {
      const sshUrl = `git@github.com:${match[1]}.git`;
      runGit(projectDir, ['remote', 'set-url', 'origin', sshUrl]);
    }
  } catch {
    // No remote or other issue — skip silently
  }
}
...
function create(repo, workerIndex, branch, baseBranch) {
  ...
  // Ensure SSH remote before any network operations
  ensureSshRemote(mainDir);
