...
async function classifyMessage(app, channel, threadTs, messages, context) {
  ...
  const prompt = [
    ...
    '  You MUST include "project_name" with just the repo name (not the full URL).',
    '  Extract from: "git@github.com:org/name.git" → "name",',
    '  "https://github.com/org/name" → "name", or a bare name like "myapp".',
    ...
    '  "git@github.com:gulati8/myapp.git" → project_name: "myapp"',
...
async function handleClassification(app, threadContext, classification) {
  ...
  if (classification.action === 'onboard' || classification.action === 'create') {
    if (!classification.project_name) {
      ...
      await reporter.post(classification.message || "What's the project name or GitHub URL?");
