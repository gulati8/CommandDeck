...
class Mission {
  ...
  async start() {
    ...
    try {
      ...
      // Retry up to 3 times (500ms apart) in case of filesystem sync delay
      let retries = 3;
      while (retries > 0) {
        this.state = await state.readMission(this.repo, this.missionId);
        if (this.state.work_items?.length) break;
        retries--;
  ...
  async revisePlan(feedback) {
    ...
    // Reload state after Picard rewrites mission.json
    let retries = 3;
    while (retries > 0) {
      this.state = await state.readMission(this.repo, this.missionId);
      if (this.state.work_items?.length) break;
  ...
  static async followUp(opts) {
    ...
    try {
      ...
      while (retries > 0) {
        mission.state = await state.readMission(repo, mission.missionId);
        if (mission.state.work_items?.length) break;
        retries--;
        if (retries > 0) await new Promise(r => setTimeout(r, 500));
      }
