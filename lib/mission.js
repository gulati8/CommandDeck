'use strict';

const { execFileSync } = require('child_process');
const fs = require('fs');
const path = require('path');

const state = require('./state');
const worktreeLib = require('./worktree');
const worker = require('./worker');
const pr = require('./pr');
const risk = require('./risk');
const { readEvidence } = require('./evidence');
const { logEvent } = require('./observability');

function runGit(cwd, args) {
  return execFileSync('git', args, { cwd, stdio: 'pipe', encoding: 'utf-8' });
}

// Sync Picard's mission.json file output into SQLite
// Picard writes objectives to disk; the DB needs them for readMission()
async function syncMissionFile(repo, missionId) {
  const filePath = state.missionPath(repo, missionId);
  try {
    const raw = JSON.parse(fs.readFileSync(filePath, 'utf-8'));
    if (raw.work_items?.length) {
      await state.withMissionLock(repo, missionId, (s) => {
        s.work_items = raw.work_items;
        return s;
      });
      return true;
    }
  } catch { /* file missing or malformed — fall through */ }
  return false;
}

class Mission {
  constructor(repo, prompt, context = {}) {
    this.repo = repo;
    this.prompt = prompt;
    this.channel = context.channel || null;
    this.threadTs = context.threadTs || null;
    this.reporter = context.reporter || noopReporter;
    this.missionId = null;
    this.state = null;
  }

  // Main entry point — runs the full mission lifecycle
  async start() {
    // 0. Validate project directory exists
    const projectDir = worktreeLib.projectPath(this.repo);
    if (!fs.existsSync(projectDir)) {
      const available = listAvailableProjects();
      const list = available.length
        ? `Available projects: ${available.join(', ')}`
        : 'No projects found. Clone a repo first.';
      await this.reporter.post(`🔴 Project "${this.repo}" not found.\n${list}`);
      throw new Error(`Project directory not found: ${projectDir}`);
    }

    // 1. Create mission state (status: 'planning')
    this.state = await state.createMission(this.repo, {
      description: this.prompt,
      slackChannel: this.channel,
      slackThreadTs: this.threadTs
    });
    this.missionId = this.state.mission_id;

    logEvent('mission.created', {
      mission_id: this.missionId,
      repo: this.repo,
      status: 'planning'
    });

    await this.reporter.post(
      `🖖 Mission acknowledged: "${this.prompt}"\n` +
      `  Project: ${this.repo}\n` +
      `  Mission ID: ${this.missionId}\n` +
      `  Captain Picard is planning the mission...`
    );

    try {
      // 2. Picard decomposes the mission
      await this.decompose();

      // Sync Picard's file output into SQLite, then reload
      await syncMissionFile(this.repo, this.missionId);
      this.state = await state.readMission(this.repo, this.missionId);

      if (!this.state.work_items?.length) {
        await this.reporter.post('⚠️ Picard produced no objectives. Mission aborted.');
        await this.setStatus('failed');
        return this.state;
      }

      // Guard against over-decomposition
      const config = state.loadProjectConfig(this.repo);
      const maxObjectives = config.max_objectives || 10;
      if (this.state.work_items.length > maxObjectives) {
        await this.reporter.post(
          `⚠️ Picard created ${this.state.work_items.length} objectives (max ${maxObjectives}). Mission aborted.`
        );
        await this.setStatus('failed');
        return this.state;
      }

      // Report the plan for approval
      const planMsgTs = await this.reportPlanForApproval();

      // Store plan message ts for reaction tracking
      await state.withMissionLock(this.repo, this.missionId, (s) => {
        s.plan_message_ts = planMsgTs;
        return s;
      });
      this.state.plan_message_ts = planMsgTs;

      // CLI mode (no channel) or follow-up: auto-approve
      if (!this.channel) {
        await this.approvePlan();
        return this.state;
      }

      // Slack mode: pause for user approval
      await this.setStatus('pending_approval');
      return this.state;
    } catch (err) {
      // Mission failed unexpectedly — mark failed and report to user
      await this.reporter.post(`🔴 Mission failed: ${err.message}`);
      try { await this.setStatus('failed'); } catch { /* state may not exist */ }
      throw err;
    }

    return this.state;
  }

  // Resume a pending-approval mission
  async resume() {
    this.state = await state.readMission(this.repo, this.missionId);
    if (!this.state) throw new Error(`Mission ${this.missionId} not found`);

    if (this.state.status !== 'pending_approval') {
      throw new Error(`Mission is ${this.state.status}, not pending_approval`);
    }

    await this.reporter.post('▶️ Plan approved — starting work...');
    await this.approvePlan();
    return this.state;
  }

  // Set mission status atomically
  async setStatus(newStatus) {
    await state.withMissionLock(this.repo, this.missionId, (s) => {
      s.status = newStatus;
      return s;
    });
    this.state.status = newStatus;

    logEvent('mission.status', {
      mission_id: this.missionId,
      repo: this.repo,
      status: newStatus
    });
  }

  // Invoke Picard to decompose the mission into objectives
  async decompose() {
    const stateDir = state.missionDir(this.repo, this.missionId);
    const projectDir = worktreeLib.projectPath(this.repo);
    const config = state.loadProjectConfig(this.repo);

    const prompt = [
      `You are Captain Picard. Decompose this mission into phased objectives.`,
      ``,
      `Mission: "${this.prompt}"`,
      `Repo: ${this.repo}`,
      `Mission ID: ${this.missionId}`,
      `State directory: ${stateDir}`,
      ``,
      `Read the project's CLAUDE.md, docs/adr/, and any existing code to understand context.`,
      `Check ~/.commanddeck/playbooks/ for relevant templates.`,
      ``,
      `IMPORTANT — Right-size the decomposition:`,
      `For simple, well-defined tasks (single endpoint, single file change, bug fix),`,
      `create exactly ONE objective. Only create multiple objectives when the task`,
      `genuinely requires parallel or phased work. Aim for the minimum number of`,
      `objectives needed. Never exceed 10 objectives per mission.`,
      ``,
      `Write your decomposition to ${stateDir}/mission.json — update the work_items array.`,
      `Write a Captain's Log entry to ${stateDir}/captains-log.md.`,
      ``,
      `Each objective needs: id, title, description, status (always "pending"),`,
      `phase, parallel_group, depends_on, assigned_to, risk_flags, context_sources.`,
      `All objectives MUST have status "pending". Readiness is computed automatically from depends_on.`,
      ``,
      `Set risk_flags based on file patterns: auth, security, migration, ci-workflow, infra, dependency.`,
      `Assign specialists: borg for implementation, scotty for architecture, worf for security,`,
      `spock for testing, geordi for infra, mr-data for data modeling.`,
    ].join('\n');

    const model = config.model_overrides?.['captain-picard'] || 'claude-opus-4-6';

    console.log('[captain-picard] Decomposing mission — this may take a few minutes...');
    const heartbeat = setInterval(() => {
      console.log('[captain-picard] Still planning...');
    }, 15000);

    let result;
    try {
      result = await worker.executeSpecialist(
        projectDir, 'captain-picard', prompt, this.state,
        { model }
      );
    } finally {
      clearInterval(heartbeat);
    }
    console.log(`[captain-picard] Decompose finished (exit code ${result.code})`);

    await state.addSessionLog(this.repo, this.missionId, {
      session_id: `sess-${Date.now()}`,
      started_at: new Date().toISOString(),
      ended_at: new Date().toISOString(),
      agent: 'captain-picard',
      objective: 'mission-planning',
      exit_code: result.code
    });

    return result;
  }

  // Report the mission plan and request approval
  async reportPlanForApproval() {
    const items = this.state.work_items;

    let report = `📋 *Mission plan — ${items.length} objective(s):*\n`;
    for (const item of items) {
      const flags = item.risk_flags?.length ? ` ⚠️ ${item.risk_flags.join(', ')}` : '';
      report += `  ${item.id}. ${item.title} (Phase ${item.phase}, ${item.assigned_to})${flags}\n`;
    }

    if (this.channel) {
      report += `\nReact to approve:\n`;
      report += `  :white_check_mark: to approve and start work\n`;
      report += `  :x: to abort the mission\n`;
      report += `  Or reply in this thread to revise the plan`;
    }

    return await this.reporter.post(report);
  }

  // Approve the plan and run the mission
  async approvePlan() {
    this.state = await state.readMission(this.repo, this.missionId);

    await this.setStatus('in_progress');
    this.ensureIntegrationBranch();

    const loopResult = await this.workLoop();

    if (shouldOpenPR(this.state)) {
      try {
        const prResult = await pr.create(this.state, { reporter: this.reporter });
        this.state.pr = { number: prResult.number, url: prResult.url, status: 'open' };
        await this.setStatus('review');
      } catch (err) {
        await this.reporter.post(`⚠️ Failed to create PR: ${err.message}`);
        await this.setStatus('failed');
      }
    }

    return this.state;
  }

  // Reject the plan — abort the mission
  async rejectPlan() {
    await this.setStatus('aborted');
    await this.reporter.post('🛑 Mission aborted.');
    return this.state;
  }

  // Revise the plan based on user feedback
  async revisePlan(feedback) {
    await this.setStatus('planning');
    await this.reporter.post('🔄 Revising plan based on your feedback...');

    await this.decomposeWithFeedback(feedback);

    // Sync Picard's file output into SQLite, then reload
    await syncMissionFile(this.repo, this.missionId);
    this.state = await state.readMission(this.repo, this.missionId);

    if (!this.state.work_items?.length) {
      await this.reporter.post('⚠️ Picard produced no objectives on revision. Mission aborted.');
      await this.setStatus('failed');
      return this.state;
    }

    const planMsgTs = await this.reportPlanForApproval();
    await state.withMissionLock(this.repo, this.missionId, (s) => {
      s.plan_message_ts = planMsgTs;
      return s;
    });
    this.state.plan_message_ts = planMsgTs;
    await this.setStatus('pending_approval');
    return { state: this.state, planMsgTs };
  }

  // Decompose with feedback — Picard revises the plan
  async decomposeWithFeedback(feedback) {
    const stateDir = state.missionDir(this.repo, this.missionId);
    const projectDir = worktreeLib.projectPath(this.repo);
    const config = state.loadProjectConfig(this.repo);

    const previousPlan = (this.state.work_items || [])
      .map(w => `- ${w.id}: ${w.title} (${w.assigned_to})`)
      .join('\n') || 'None';

    const prompt = [
      `You are Captain Picard. The user has requested changes to the mission plan.`,
      ``,
      `Mission: "${this.prompt}"`,
      `Repo: ${this.repo}`,
      `Mission ID: ${this.missionId}`,
      `State directory: ${stateDir}`,
      ``,
      `Previous plan:`,
      previousPlan,
      ``,
      `User feedback: "${feedback}"`,
      ``,
      `Read the project's CLAUDE.md, docs/adr/, and any existing code to understand context.`,
      `Revise the plan based on the user's feedback. Replace the work_items in mission.json.`,
      ``,
      `IMPORTANT — Right-size the decomposition:`,
      `For simple, well-defined tasks (single endpoint, single file change, bug fix),`,
      `create exactly ONE objective. Only create multiple objectives when the task`,
      `genuinely requires parallel or phased work.`,
      ``,
      `Write your revised decomposition to ${stateDir}/mission.json — update the work_items array.`,
      `Update the Captain's Log at ${stateDir}/captains-log.md.`,
      ``,
      `Each objective needs: id, title, description, status (always "pending"),`,
      `phase, parallel_group, depends_on, assigned_to, risk_flags, context_sources.`,
      `All objectives MUST have status "pending". Readiness is computed automatically from depends_on.`,
      ``,
      `Set risk_flags based on file patterns: auth, security, migration, ci-workflow, infra, dependency.`,
    ].join('\n');

    const model = config.model_overrides?.['captain-picard'] || 'claude-opus-4-6';

    console.log('[captain-picard] Revising plan — this may take a few minutes...');
    const heartbeat = setInterval(() => {
      console.log('[captain-picard] Still revising...');
    }, 15000);

    let result;
    try {
      result = await worker.executeSpecialist(
        projectDir, 'captain-picard', prompt, this.state,
        { model }
      );
    } finally {
      clearInterval(heartbeat);
    }
    console.log(`[captain-picard] Revision finished (exit code ${result.code})`);

    await state.addSessionLog(this.repo, this.missionId, {
      session_id: `sess-${Date.now()}`,
      started_at: new Date().toISOString(),
      ended_at: new Date().toISOString(),
      agent: 'captain-picard',
      objective: 'plan-revision',
      exit_code: result.code
    });

    return result;
  }

  // The main work loop — returns 'done' or 'failed'
  async workLoop() {
    const config = state.loadProjectConfig(this.repo);
    const maxWorkers = this.state.safety?.max_parallel_workers || config.max_workers || 3;

    while (true) {
      this.state = await state.readMission(this.repo, this.missionId);

      // Check safety limits
      const safetyCheck = checkSafetyLimits(this.state);
      if (safetyCheck.blocked) {
        await this.reporter.post(`🛑 Safety limit reached: ${safetyCheck.reason}. Mission paused.`);
        await this.setStatus('paused');
        return 'paused';
      }

      // Evaluate pending objectives — transitions pending → queued or upstream_failed
      const queued = state.evaluateReadiness(this.state);
      const inProgress = state.getItemsByStatus(this.state, 'in_progress');
      const pending = state.getItemsByStatus(this.state, 'pending');

      // Persist any status transitions (queued, upstream_failed) that evaluateReadiness made
      const upstreamFailed = state.getItemsByStatus(this.state, 'upstream_failed');
      for (const item of [...queued, ...upstreamFailed]) {
        await state.updateItemStatus(this.repo, this.missionId, item.id, item.status);
      }

      // All done?
      if (queued.length === 0 && inProgress.length === 0 && pending.length === 0) {
        const failed = state.getItemsByStatus(this.state, 'failed');
        const cascaded = upstreamFailed;
        if (failed.length > 0 || cascaded.length > 0) {
          await this.setStatus('failed');
          const allFailed = [...failed, ...cascaded];
          await this.reporter.post(
            `🔴 Mission incomplete. ${allFailed.length} objective(s) failed:\n` +
            allFailed.map(f => `  - ${f.id}: ${f.title}${f.status === 'upstream_failed' ? ' (upstream failed)' : ''}`).join('\n')
          );
          return 'failed';
        } else {
          await this.setStatus('merging');
          await this.reporter.post('✅ All objectives complete. Preparing PR...');
          return 'done';
        }
      }

      // Nothing queued but some still in progress or pending — wait
      if (queued.length === 0) {
        if (pending.length > 0 && inProgress.length === 0) {
          // Deadlock: pending items exist but nothing can run
          await this.setStatus('failed');
          await this.reporter.post(
            `🔴 Deadlock: ${pending.length} objective(s) stuck pending with unresolvable dependencies.`
          );
          return 'failed';
        }
        return 'done';
      }

      const batch = queued.slice(0, maxWorkers);

      // Launch batch in parallel
      await this.executeBatch(batch);

      // Merge completed branches into integration
      await this.mergeCompleted();

      // Run mandatory reviews for risk-flagged objectives
      await this.runMandatoryReviews();
    }
  }

  // Execute a batch of objectives in parallel worktrees
  async executeBatch(batch) {
    const projectDir = worktreeLib.projectPath(this.repo);
    const config = state.loadProjectConfig(this.repo);

    // Mark all as in_progress and create worktrees
    const workers = [];
    for (let i = 0; i < batch.length; i++) {
      const obj = batch[i];
      const branch = `commanddeck/${this.missionId}/${obj.id}`;

      // Update status atomically
      await state.updateItemStatus(this.repo, this.missionId, obj.id, 'in_progress', {
        git_branch: branch,
        worker_index: i,
        started_at: new Date().toISOString()
      });
      obj.status = 'in_progress';
      obj.git_branch = branch;

      // Create worktree — follow-ups branch from integration, not default
      const baseBranch = this.state.is_follow_up ? this.state.integration_branch : undefined;
      const wtPath = worktreeLib.create(this.repo, i, branch, baseBranch);

      await this.reporter.post(
        `⚡ ${obj.assigned_to}${batch.length > 1 ? `-${i}` : ''} → ${obj.title}`
      );

      // Increment session count
      await state.incrementSessionCount(this.repo, this.missionId);

      // Determine model from config
      const model = config.model_overrides?.[obj.assigned_to] || undefined;

      workers.push(
        worker.execute(wtPath, obj, this.state, { model })
          .then(result => ({ ...result, objective: obj }))
          .catch(err => ({
            success: false,
            objective: obj,
            error: err.message
          }))
      );
    }

    // Wait for all workers to complete
    const results = await Promise.all(workers);

    // Process results atomically
    for (const result of results) {
      const obj = result.objective;

      if (result.success) {
        // Detect risk flags from actual files changed (evidence bundle)
        const evidence = readEvidence(this.repo, this.missionId, obj.id);
        const actualFiles = evidence
          ? [
            ...(evidence.files_changed?.created || []),
            ...(evidence.files_changed?.modified || []),
            ...(evidence.files_changed?.deleted || [])
          ]
          : [];
        const detectedFlags = actualFiles.length > 0
          ? risk.detectRiskFlags(actualFiles, this.repo)
          : [];
        // Merge with any flags Picard set (deduplicated)
        const existingFlags = obj.risk_flags || [];
        const mergedFlags = [...new Set([...existingFlags, ...detectedFlags])];

        await state.updateItemStatus(this.repo, this.missionId, obj.id, 'done', {
          completed_at: new Date().toISOString(),
          evidence_path: `artifacts/evidence-${obj.id}.json`,
          risk_flags: mergedFlags.length > 0 ? mergedFlags : undefined
        });
        obj.status = 'done';
        obj.risk_flags = mergedFlags.length > 0 ? mergedFlags : obj.risk_flags;
        await this.reporter.post(`✅ ${obj.id} complete: ${obj.title}`);
      } else {
        const error = result.error || `Worker exited with code ${result.code}`;
        await state.updateItemStatus(this.repo, this.missionId, obj.id, 'failed', {
          error
        });
        obj.status = 'failed';
        await this.reporter.post(`🔴 ${obj.id} failed: ${obj.title} — ${error}`);
      }
    }

    // Clean up worktrees
    for (let i = 0; i < batch.length; i++) {
      worktreeLib.remove(this.repo, i);
    }
  }

  // Merge completed objective branches into the integration branch
  async mergeCompleted() {
    const projectDir = worktreeLib.projectPath(this.repo);
    const integrationBranch = this.state.integration_branch;

    const completedItems = this.state.work_items.filter(
      w => w.status === 'done' && w.git_branch && !w.merged
    );

    if (completedItems.length === 0) return;

    // Checkout integration branch
    runGit(projectDir, ['checkout', integrationBranch]);

    for (const obj of completedItems) {
      try {
        runGit(projectDir, ['merge', obj.git_branch, '--no-edit']);
        obj.merged = true;
        await this.reporter.post(`🔀 Merged ${obj.id} to integration branch — clean merge.`);
      } catch {
        // Merge conflict — invoke O'Brien
        await this.reporter.post(`🔀 Merge conflict on ${obj.id} — O'Brien resolving...`);
        try {
          await this.invokeOBrien(obj.git_branch);
          obj.merged = true;
          await this.reporter.post(`🔀 O'Brien resolved conflict for ${obj.id}.`);
        } catch (err) {
          await this.reporter.post(`🔴 O'Brien could not resolve conflict for ${obj.id}: ${err.message}`);
          // Abort the merge so we can continue
          try {
            runGit(projectDir, ['merge', '--abort']);
          } catch { /* nothing to abort */ }
        }
      }
    }

    await state.writeMission(this.repo, this.missionId, this.state);

    // Return to default branch
    try {
      runGit(projectDir, ['checkout', this.state.default_branch]);
    } catch { /* may already be there */ }
  }

  // Invoke O'Brien to resolve a merge conflict
  async invokeOBrien(branch) {
    const projectDir = worktreeLib.projectPath(this.repo);
    const config = state.loadProjectConfig(this.repo);
    const model = config.model_overrides?.['obrien'] || 'claude-opus-4-6';

    const result = await worker.executeSpecialist(
      projectDir, 'obrien',
      `Merge conflict merging branch ${branch} into ${this.state.integration_branch}. ` +
      `Read briefings in the mission state directory to understand intent of both branches. ` +
      `Resolve conflicts preserving both sides' intent. ` +
      `Run git add -A && git commit to complete the merge. ` +
      `Then run tests to verify nothing broke.`,
      this.state,
      { model }
    );

    if (!result.success) {
      throw new Error(`O'Brien failed with exit code ${result.code}`);
    }
  }

  // Run mandatory specialist reviews for risk-flagged objectives
  async runMandatoryReviews() {
    const projectDir = worktreeLib.projectPath(this.repo);
    const config = state.loadProjectConfig(this.repo);

    for (const obj of this.state.work_items) {
      if (obj.status !== 'done' || !obj.risk_flags?.length) continue;

      const reviewed = obj.reviewed_by || [];
      const reviewers = risk.getMandatoryReviewers(obj.risk_flags);
      for (const reviewer of reviewers) {
        if (reviewer === 'human') continue; // Handled by checkpoints
        if (reviewed.includes(reviewer)) continue;

        await this.reporter.post(
          `🛡️ ${reviewer} reviewing ${obj.id} (risk flags: ${obj.risk_flags.join(', ')})...`
        );

        const model = config.model_overrides?.[reviewer] || undefined;
        const reviewPrompt = buildReviewPrompt(reviewer, obj, this.state);
        const result = await worker.executeSpecialist(
          projectDir, reviewer, reviewPrompt, this.state, { model }
        );

        // Persist review completion on the work item
        await state.updateItemStatus(
          this.repo, this.missionId, obj.id, obj.status,
          { reviewed_by: [...reviewed, reviewer] }
        );
        reviewed.push(reviewer);
        await state.incrementSessionCount(this.repo, this.missionId);

        if (result.success) {
          await this.reporter.post(`✅ ${reviewer}: Review complete for ${obj.id}.`);
        } else {
          await this.reporter.post(`⚠️ ${reviewer}: Review had issues for ${obj.id}.`);
        }
      }
    }
  }

  // Follow-up decomposition — Picard prompt tailored for iteration
  async decomposeFollowUp(prompt, parentDescription) {
    const stateDir = state.missionDir(this.repo, this.missionId);
    const projectDir = worktreeLib.projectPath(this.repo);
    const config = state.loadProjectConfig(this.repo);

    const completedSummary = this.state.work_items
      .filter(w => w.status === 'done')
      .map(w => `- ${w.id}: ${w.title}`)
      .join('\n') || 'None yet';

    const picardPrompt = [
      'You are Captain Picard. This is a FOLLOW-UP mission based on user feedback.',
      '',
      `Original task: "${parentDescription}"`,
      `User feedback / follow-up request: "${prompt}"`,
      `Repo: ${this.repo}`,
      `Mission ID: ${this.missionId}`,
      `State directory: ${stateDir}`,
      '',
      'Previously completed objectives:',
      completedSummary,
      '',
      'IMPORTANT:',
      '- Workers will branch FROM the integration branch, not from main.',
      '- The existing code changes are already on the integration branch.',
      '- Most follow-up requests need exactly ONE objective.',
      '- Only create multiple objectives if the feedback genuinely requires parallel work.',
      '',
      `Write your decomposition to ${stateDir}/mission.json — update the work_items array.`,
      `Write a Captain's Log entry to ${stateDir}/captains-log.md.`,
      '',
      'Each objective needs: id, title, description, status (always "pending"),',
      'phase, parallel_group, depends_on, assigned_to, risk_flags, context_sources.',
      'All objectives MUST have status "pending". Readiness is computed automatically from depends_on.',
    ].join('\n');

    const model = config.model_overrides?.['captain-picard'] || 'claude-opus-4-6';

    console.log('[captain-picard] Decomposing follow-up — this may take a few minutes...');
    const heartbeat = setInterval(() => {
      console.log('[captain-picard] Still planning follow-up...');
    }, 15000);

    let result;
    try {
      result = await worker.executeSpecialist(
        projectDir, 'captain-picard', picardPrompt, this.state,
        { model }
      );
    } finally {
      clearInterval(heartbeat);
    }
    console.log(`[captain-picard] Follow-up decompose finished (exit code ${result.code})`);

    await state.addSessionLog(this.repo, this.missionId, {
      session_id: `sess-${Date.now()}`,
      started_at: new Date().toISOString(),
      ended_at: new Date().toISOString(),
      agent: 'captain-picard',
      objective: 'follow-up-planning',
      exit_code: result.code
    });

    return result;
  }

  // Static method: create and run a follow-up mission on an existing branch/PR
  static async followUp(opts) {
    const {
      repo, parentMissionId, integrationBranch, prNumber, prUrl,
      originalDescription, prompt, context = {}
    } = opts;

    const reporter = context.reporter || noopReporter;

    // Create a new mission instance
    const mission = new Mission(repo, prompt, context);

    // Create mission state with follow-up metadata
    mission.state = await state.createMission(repo, {
      description: `Follow-up: ${prompt}`,
      slackChannel: context.channel || null,
      slackThreadTs: context.threadTs || null
    });
    mission.missionId = mission.state.mission_id;

    // Override integration branch and PR to reuse parent's
    await state.withMissionLock(repo, mission.missionId, (s) => {
      s.integration_branch = integrationBranch;
      s.pr = { number: prNumber, url: prUrl, status: 'open' };
      s.is_follow_up = true;
      s.parent_mission_id = parentMissionId;
      return s;
    });
    mission.state = await state.readMission(repo, mission.missionId);

    logEvent('mission.follow_up', {
      mission_id: mission.missionId,
      parent_mission_id: parentMissionId,
      repo
    });

    await reporter.post(
      `Follow-up mission ${mission.missionId} — Picard is planning...`
    );

    try {
      // Picard decomposes the follow-up
      await mission.decomposeFollowUp(prompt, originalDescription);

      // Sync Picard's file output into SQLite, then reload
      await syncMissionFile(repo, mission.missionId);
      mission.state = await state.readMission(repo, mission.missionId);

      if (!mission.state.work_items?.length) {
        await reporter.post('Picard produced no follow-up objectives.');
        await mission.setStatus('failed');
        return mission.state;
      }

      // Check for risk flags — require approval if any objectives are risky
      const hasRiskFlags = mission.state.work_items.some(
        w => w.risk_flags && w.risk_flags.length > 0
      );

      const planMsgTs = await mission.reportPlanForApproval();

      if (hasRiskFlags && context.channel) {
        await state.withMissionLock(repo, mission.missionId, (s) => {
          s.plan_message_ts = planMsgTs;
          return s;
        });
        mission.state.plan_message_ts = planMsgTs;
        await mission.setStatus('pending_approval');
        return mission.state;
      }

      await mission.setStatus('in_progress');

      // Ensure integration branch is checked out and up to date
      const projectDir = worktreeLib.projectPath(repo);
      try {
        runGit(projectDir, ['fetch', 'origin']);
        runGit(projectDir, ['checkout', integrationBranch]);
        runGit(projectDir, ['pull', 'origin', integrationBranch]);
      } catch {
        // Branch may only exist locally
        try {
          runGit(projectDir, ['checkout', integrationBranch]);
        } catch { /* already there */ }
      }

      // Execute the work loop
      await mission.workLoop();

      // Push updated integration branch and update PR body
      if (shouldOpenPR(mission.state)) {
        try {
          runGit(projectDir, ['push', 'origin', integrationBranch]);
          // Reload state to include merged work items
          mission.state = await state.readMission(repo, mission.missionId);
          mission.state.pr = { number: prNumber, url: prUrl, status: 'open' };
          await state.writeMission(repo, mission.missionId, mission.state);
          await pr.updateBody(mission.state);
          await mission.setStatus('review');
        } catch (err) {
          await reporter.post(`Failed to update PR: ${err.message}`);
          await mission.setStatus('failed');
        }
      }
    } catch (err) {
      await reporter.post(`Follow-up mission failed: ${err.message}`);
      try { await mission.setStatus('failed'); } catch { /* state may not exist */ }
      throw err;
    }

    return mission.state;
  }

  // Ensure the integration branch exists
  ensureIntegrationBranch() {
    const projectDir = worktreeLib.projectPath(this.repo);
    const integrationBranch = this.state.integration_branch;

    try {
      runGit(projectDir, ['rev-parse', '--verify', integrationBranch]);
    } catch {
      // Branch doesn't exist — create it from default branch
      runGit(projectDir, ['branch', integrationBranch, this.state.default_branch]);
    }
  }
}

function shouldOpenPR(missionState) {
  if (!missionState) return false;
  if (missionState.status !== 'merging') return false;
  const terminalStatuses = new Set(['done', 'upstream_failed', 'skipped']);
  const nonTerminal = missionState.work_items.some(item => !terminalStatuses.has(item.status));
  if (nonTerminal) return false;
  // At least one objective must have actually completed
  return missionState.work_items.some(item => item.status === 'done');
}

// Build a review prompt for a specialist
function buildReviewPrompt(reviewer, objective, mission) {
  const stateDir = state.missionDir(mission.repo, mission.mission_id);

  const prompts = {
    worf: `Review objective ${objective.id} (${objective.title}) for security issues. ` +
      `Risk flags: ${objective.risk_flags.join(', ')}. ` +
      `Read the evidence bundle at ${stateDir}/artifacts/evidence-${objective.id}.json ` +
      `and review the code changes. Write your security review to ` +
      `${stateDir}/briefings/worf-review.json.`,

    geordi: `Review objective ${objective.id} (${objective.title}) for infrastructure safety. ` +
      `Risk flags: ${objective.risk_flags.join(', ')}. ` +
      `Read the evidence bundle and review configuration changes. Write your review to ` +
      `${stateDir}/briefings/geordi-review.json.`,

    spock: `Review objective ${objective.id} (${objective.title}) for test coverage and dependency safety. ` +
      `Risk flags: ${objective.risk_flags.join(', ')}. ` +
      `Verify no breaking changes. Write your report to ` +
      `${stateDir}/briefings/spock-report.json.`
  };

  return prompts[reviewer] || `Review objective ${objective.id} for ${objective.risk_flags.join(', ')} concerns.`;
}

// Check safety limits
function checkSafetyLimits(missionState) {
  const { safety } = missionState;

  if (safety.session_count >= safety.max_sessions) {
    return { blocked: true, reason: `Session limit reached (${safety.max_sessions})` };
  }

  return { blocked: false };
}

// List available projects (directories in the projects dir)
function listAvailableProjects() {
  const projectDir = process.env.COMMANDDECK_PROJECT_DIR
    || path.join(process.env.HOME, 'projects');
  try {
    return fs.readdirSync(projectDir, { withFileTypes: true })
      .filter(d => d.isDirectory())
      .map(d => d.name);
  } catch {
    return [];
  }
}

// No-op reporter for CLI/testing when no Slack
const noopReporter = {
  post: async (msg) => { console.log(msg); }
};

module.exports = { Mission, checkSafetyLimits, shouldOpenPR, listAvailableProjects };
