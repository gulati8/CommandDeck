...
class Mission {
  ...
  async resume() {
    ...
    // Find the checkpoint item and mark it ready so the work loop executes it
  ...
  async decompose() {
    ...
    const prompt = [
      ...
      `phase, parallel_group, depends_on, assigned_to, checkpoint (boolean — true ONLY if`,
      `human approval is required before this objective runs, false otherwise), risk_flags, context_sources.`,
      ``,
  ...
  async reportPlan() {
    ...
    for (const item of items) {
      ...
      const checkpoint = item.checkpoint === true ? ' 🔒 checkpoint' : '';
  ...
  async workLoop() {
    ...
    while (true) {
      ...
      // Separate checkpoint vs non-checkpoint ready items
      const nonCheckpoint = ready.filter(r => r.checkpoint !== true);
      const checkpoint = ready.find(r => r.checkpoint === true);

      // If only checkpoint items remain, pause the mission
      if (nonCheckpoint.length === 0 && checkpoint) {
      ...
      // Batch: take up to maxWorkers from non-checkpoint ready items
      const batch = (nonCheckpoint.length > 0 ? nonCheckpoint : ready).slice(0, maxWorkers);
  ...
  async decomposeFollowUp(prompt, parentDescription) {
    ...
    const picardPrompt = [
      ...
      'phase, parallel_group, depends_on, assigned_to, checkpoint (false),',
      'risk_flags, context_sources.',
    ].join('\n');
  ...
  static async followUp(opts) {
    ...
    try {
      ...
      const loopResult = await mission.workLoop();
