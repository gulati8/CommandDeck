...
const { persistHealthAlert, logEvent } = require('./observability');
...
async function patrol(mission, { reporter }) {
  ...
  for (const obj of inProgress) {
    ...
    if (checks.testFailures >= 2) {
      ...
      persistHealthAlert(mission.repo, mission.mission_id, alerts[alerts.length - 1]);
      continue;
    }
...
function runChecks(wtPath, objective) {
  return {
    minutesSinceCommit: getMinutesSinceActivity(wtPath, objective.started_at),
...
// Get minutes since last meaningful activity in a worktree.
// Uses the later of (last commit time, worker start time) so that
// pre-existing commits on the branch base don't trigger false alarms.
function getMinutesSinceActivity(wtPath, startedAt) {
  const now = Date.now();
  const workerStart = startedAt ? new Date(startedAt).getTime() : 0;
  ...
  try {
    ...
    // Use the later of commit time and worker start — if the latest commit
    // predates the worker starting, the worker just hasn't committed yet
    const baseline = Math.max(commitTime, workerStart);
    return (now - baseline) / 60000;
  } catch {
    // No commits at all — measure from worker start if available
    if (workerStart > 0) {
      return (now - workerStart) / 60000;
    }
    return 999;
  }
}
...
function startPatrol(getMissions, reporter) {
  return setInterval(async () => {
    const missionRefs = getMissions();
    for (const missionRef of missionRefs) {
      const mission = missionRef.repo && missionRef.mission_id
        ? await state.readMission(missionRef.repo, missionRef.mission_id)
        : missionRef;
      if (mission?.status === 'in_progress') {
        try {
          await patrol(mission, { reporter });
        } catch (err) {
          logEvent('health.error', {
            mission_id: mission.mission_id,
            repo: mission.repo,
            message: err.message
          });
