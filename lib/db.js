...
function runMigrations(db) {
  ...
  if (!applied.has('v5-objective-statuses')) {
    db.transaction(() => {
      // Normalize legacy ready/blocked statuses to pending
      db.prepare("UPDATE objectives SET status = 'pending' WHERE status IN ('ready', 'blocked')").run();
      // Unstick missions that are merging but still have pending objectives
      db.prepare(`
        UPDATE missions SET status = 'in_progress'
        WHERE status = 'merging'
        AND id IN (SELECT DISTINCT mission_id FROM objectives WHERE status = 'pending')
      `).run();
      db.prepare("INSERT INTO _migrations (name) VALUES ('v5-objective-statuses')").run();
    })();
  }
}
...
// Normalize legacy objective statuses from Picard output
function normalizeObjectiveStatus(status) {
  ...
  const tx = db.transaction(() => {
    ...
    for (const obj of objectives) {
      insert.run(
        ...
        normalizeObjectiveStatus(obj.status) || 'pending', obj.phase || 1, obj.parallel_group || null,
            ...
            if (m.work_items?.length) {
              ...
              for (const w of m.work_items) {
                insertObj.run(
                  ...
                  normalizeObjectiveStatus(w.status) || 'pending', w.phase || 1, w.parallel_group || null,
