...
// Load agent identity from agents/<name>.md
// Returns { identity, tools, model } or defaults for unknown agents
function loadAgentIdentity(agentName) {
  ...
  try {
    ...
    return { identity: '', tools: null, model: null };
  }
}
...
function execute(worktreePath, objective, mission, { model, onStdout, onStderr } = {}) {
  ...
  const agent = loadAgentIdentity(objective.assigned_to);

  const prompt = [
    agent.identity,
    ``,
    `Executing objective ${objective.id}: ${objective.title}`,
    ``,
  ...
  const agentModel = model || getModelForAgent(objective.assigned_to, mission, agent.model) || DEFAULT_MODEL;
  const allowedTools = agent.tools ? agent.tools.join(',') : DEFAULT_TOOLS;
  ...
  return new Promise((resolve, reject) => {
    const proc = spawn('claude', [
      ...
      env: {
        ...
        COMMANDDECK_REPO: mission.repo,
        COMMANDDECK_OBJECTIVE_ID: objective.id
      },
    ...
    proc.stdout.on('data', (data) => {
      ...
      streamLog(objective.assigned_to, objective.id, 'out', chunk, logCtx);
    });
    ...
    proc.stderr.on('data', (data) => {
      ...
      streamLog(objective.assigned_to, objective.id, 'err', chunk, logCtx);
    });
...
function executeSpecialist(projectDir, agent, prompt, mission, { model } = {}) {
  ...
  const agentId = loadAgentIdentity(agent);
  ...
  const agentModel = model || getModelForAgent(agent, mission, agentId.model) || DEFAULT_MODEL;
  ...
  return new Promise((resolve, reject) => {
    ...
    proc.stdout.on('data', (data) => {
      const chunk = data.toString();
      stdout += chunk;
      streamLog(agent, 'specialist', 'out', chunk, logCtx);
    });
    proc.stderr.on('data', (data) => {
      const chunk = data.toString();
      stderr += chunk;
      streamLog(agent, 'specialist', 'err', chunk, logCtx);
    });
...
// Priority: project config override > frontmatter model > DEFAULT_MODEL
function getModelForAgent(agentName, mission, frontmatterModel) {
  // Check project config for model overrides (highest priority)
  ...
  // Frontmatter model from agents/*.md (lowest priority default)
  if (frontmatterModel) {
...
// Stream worker output to container logs and telemetry.db in real time
function streamLog(agent, objectiveId, stream, chunk, logCtx) {
  ...
  // Write to telemetry.db for dashboard visibility
  if (logCtx) {
    try {
      tdb.writeLog({
        mission_id: logCtx.mission_id,
        repo: logCtx.repo,
        source: stream === 'err' ? 'stderr' : 'stdout',
        agent: logCtx.agent || agent,
        objective_id: logCtx.objective_id || objectiveId,
        level: stream === 'err' ? 'warn' : 'info',
        message: chunk.substring(0, 2000),
        data: {}
      });
    } catch {
      // Don't let telemetry failures break worker output
    }
  }
}
...
// Lightweight Picard call for message triage — no mission context needed
function executeTriage(prompt, { model } = {}) {
  ensureClaude();
  const agentId = loadAgentIdentity('captain-picard');
  const fullPrompt = [agentId.identity, '', prompt].join('\n');
  const agentModel = model || agentId.model || DEFAULT_MODEL;
  ...
  return new Promise((resolve, reject) => {
    const proc = spawn('claude', [
      ...
      env: { ...process.env, COMMANDDECK_AGENT: 'captain-picard' },
      stdio: ['pipe', 'pipe', 'pipe'],
      timeout: 120000
    });
